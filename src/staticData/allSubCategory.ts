import hair from "@/assets/images/treatment/Hair Transplantation.png";
import fecial from "@/assets/images/treatment/Facial Aesthetic Surgery.png";
import body from "@/assets/images/treatment/Body Contouring.png";
import breastCategory from "@/assets/images/treatment/Breast Surgery.png";
// hair
import Beard from "@/assets/images/subnested/Beard Transplant.png";
import prp from "@/assets/images/subnested/PRP Hair Treatment.png";
import DHI from "@/assets/images/subnested/DHI Hair Transplant.png";
import Eyebrow from "@/assets/images/subnested/Eyebrow Transplant.png";
import FUE from "@/assets/images/subnested/FUE Hair Transplant.png";
import Mesotherapy from "@/assets/images/subnested/Mesotherapy for Hair.png";
import Moustache from "@/assets/images/subnested/Moustache Transplant.png";
import double from "@/assets/images/subnested/Double Chin Liposuction.png";
// body contouring
import arm from "@/assets/images/subnested/Arm lift.png";
import brazilianad from "@/assets/images/subnested/Brazilian Butt Lift.png";
import breast from "@/assets/images/subnested/Breast lift (Mastopexy).png";
import liposuction from "@/assets/images/subnested/Liposuction.png";
import tummy from "@/assets/images/subnested/Tummy tuck (Abdominoplasty).png";
import thighlift from "@/assets/images/subnested/Thigh lift.png";
// fecial
import rhinoplasty from "@/assets/images/subnested/Rhinoplasty (Nose reshaping).png";
import brow from "@/assets/images/subnested/Brow lift.png";
import neck from "@/assets/images/subnested/Neck lift.png";
import otoplasty from "@/assets/images/subnested/Otoplasty.png";
import ble from "@/assets/images/subnested/Blepharoplasty (Upper and Lower).png";
// breast
import breatSubCategory from "@/assets/images/subCategory/Breast augmentation (implants).png";
// genital
import genital from "@/assets/images/treatment/Genital Aesthetic.png";
import labiaplastyGenital from "@/assets/images/subnested/Labiaplasty _ Barbie Look.png";
import penileGenital from "@/assets/images/subnested/PeyronieDisease.png";
import implantology from "@/assets/images/subCategory/implantology.png";
import den64 from "@/assets/images/subnested/All on 6 Dental Implants.png";
import destigeneral from "@/assets/images/subCategory/General Dentistry.png";
import rootcanal from "@/assets/images/subnested/Root Canal Therapy.png";
import cosmeticdentistry from "@/assets/images/subCategory/Cosmetic Dentistry.png";
import laminateveneers from "@/assets/images/subnested/Laminate Veneers.png";
import restoryDenstiry from "@/assets/images/subCategory/Restorative Dentistry.png";
import dentistchoosing from "@/assets/images/subnested/Dental Crowns.png";
import periodontics from "@/assets/images/subnested/Periodontics.png";
import hollywood from "@/assets/images/subnested/Holleywood Smile.png";
import teethwhiting from "@/assets/images/subnested/Teeth Whitening.png";
import orthodontics from "@/assets/images/subnested/Orthodontics.png";
import clearaligners from "@/assets/images/subnested/ClearAligners.png";
import gumgrafting from "@/assets/images/subnested/Gum Grafting.png";
import lasergumtreatments from "@/assets/images/subnested/Periodontics.png";
import oralsurgery from "@/assets/images/subnested/Oral Surgery.png";
import bonegrafting from "@/assets/images/subnested/Bone Grafting.png";
import glocama from "@/assets/images/subnested/Glaucoma treatment.png";
import intraocularImg from "@/assets/images/subnested/Intraocular lens (IOL) implantation.png";
import eyedesis from "@/assets/images/subCategory/Eye disease treatments.png";
import surgical from "@/assets/images/subCategory/Surgical implant procedures.png";
import diabetic from "@/assets/images/subnested/Diabetic retinopathy treatment.png";
import corneal from "@/assets/images/subnested/Corneal transplantation.png";
import maleFertility from "@/assets/images/subnested/Male Infertility.png";
import asissfertility from "@/assets/images/subnested/Fertility assisted reproduction.png";
import malesecual from "@/assets/images/subnested/Male sexual health issues.png";
import malesexual from "@/assets/images/subnested/Male sexual health issues.png";
import peyroniesdisease from "@/assets/images/subnested/PeyronieDisease.png";
import dyfunc from "@/assets/images/subnested/Erectile Dysfunction.png";
import theregemed from "@/assets/images/subCategory/therapy Regenerative Medicine.jpg";
import orthopedic from "@/assets/images/subnested/Orthopedic Stem Cell Therapy.png";
import stemmen from "@/assets/images/subnested/Stem Cell Treatments for Men.png";
import neurological from "@/assets/images/subnested/Neurological Stem Cell Therapy.png";
import tipplasty from "@/assets/images/subnested/Tipplasty.png";
import almondeye from "@/assets/images/subnested/almond.png";
import facelift from "@/assets/images/subnested/Facelift.png";
import bichecktomy from "@/assets/images/subnested/Bichectomy.png";
import momymakeover from "@/assets/images/subnested/Mommy Makeover.png";
import breastreduc from "@/assets/images/subnested/Breast reduction.png";
import malebreastreduc from "@/assets/images/subnested/Male breast reduction (Gynecomastia treatment).png";
import veginal from "@/assets/images/subnested/Vaginal Tightening.png";
import implant from "@/assets/images/subnested/Implant Treatment.png";

import fillings from "@/assets/images/subnested/Fillings.png";
import rotinecheckupscleaning from "@/assets/images/subnested/Routine Check-ups and Cleanings.png";
import toothextraction from "@/assets/images/subnested/Tooth Extractions.png";
import jawsurgery from "@/assets/images/subnested/Jaw Surgery.png";
import cataract from "@/assets/images/subnested/Cataract treatment.png";
import retinaldisease from "@/assets/images/subnested/Retinal disease treatment.png";
import xrays from "@/assets/images/subnested/Dental X-rays.png";
import fet from "@/assets/images/subnested/FET (Frozen Embryo Transfer).png";
import ivf from "@/assets/images/subnested/IVF (In Vitro Fertilization).png";
import lasiksurgery from "@/assets/images/subnested/LASIK surgery.png";
import smartlens from "@/assets/images/subnested/Smart Lens in Turkey.png";
import wisdom from "@/assets/images/subnested/Wisdom Teeth Removal.png";
import dentalbonding from "@/assets/images/subnested/Dental Bonding.png";
import healthlogo from "@/assets/images/healthlogo.png";
// export const allSubCategory = (locale: string) => {
//   const isRu = locale === "ru";
//   const treatments = [
//     {
//       img: hair,
//       title: isRu ? "" : "Hair Transplantation",
//       description: isRu
//         ? ""
//         : "Tired of thinning hair or a receding hairline? Discover affordable, world-class hair transplant solutions at AZPO Health in Istanbul, Turkey. Regain your youthful appearance and confidence with our experienced medical team and state-of-the-art techniques.",
//       categoryKey: "aesthetic",
//       path: "hair-transplantation",
//     },
//     {
//       img: fecial,
//       title: isRu ? "" : "Facial Aesthetic Surgery",
//       description: isRu
//         ? ""
//         : "Is aging or genetics affecting your confidence? Discover world-class facial aesthetic treatments in Istanbul, Antalya, and Alanya for a natural, refreshed you! Contact AZPO Health today for a free consultation and reveal your best self!",
//       categoryKey: "aesthetic",
//       path: "facial-aesthetic-surgery",
//     },
//     {
//       img: body,
//       title: isRu ? "" : "Body contouring",
//       description: isRu
//         ? ""
//         : "Achieve the body of your dreams with advanced body contouring procedures in Turkey. Experience world-class medical care, personalized treatment plans, and stunning results.",
//       categoryKey: "aesthetic",
//       path: "body-contouring",
//     },
//     {
//       img: breastCategory,
//       title: isRu ? "" : "Breast Surgery",
//       description: isRu
//         ? ""
//         : "Considering breast surgery? Unlock your dream silhouette and boost your self-esteem with world-class breast surgery in Turkey. AZPO Health offers expert surgeons, luxurious care, and affordable packages. Explore your options for breast augmentation, reduction, and lift in a premier medical tourism destination.",
//       categoryKey: "aesthetic",
//       path: "breast-surgery",
//     },
//     {
//       img: genital,
//       title: isRu ? "" : "Genital Aesthetic",
//       description: isRu
//         ? ""
//         : "Considering genital aesthetic surgery to address personal concerns or enhance your intimate well-being? Turkey has become a leading destination for these specialized procedures, offering discreet, high-quality care. Discover how AZPO Health connects you with expert surgeons and all-inclusive packages for a comfortable and transformative experience.",
//       categoryKey: "aesthetic",
//       path: "genital-aesthetic",
//     },
//     {
//       img: healthlogo,
//       title: isRu ? "" : "Bariatric Surgery",
//       description: isRu
//         ? ""
//         : "Are you facing the daily struggles associated with excess weight, such as reduced mobility, fatigue, or related health conditions? Many people feel self-conscious or limited by their weight, impacting their quality of life. Finding a treatment that offers significant, long-term weight loss can seem daunting. Bariatric surgery offers a powerful tool to help individuals regain control of their health and unlock a more active, fulfilling future.",
//       categoryKey: "aesthetic",
//       path: "bariatric-surgery",
//     },
//     // dental
//     {
//       img: implantology,
//       title: isRu ? "" : "Implantology",
//       description: isRu
//         ? ""
//         : "Ready to replace missing teeth and rediscover the confidence of a complete, healthy smile? Turkey has become a global leader in dental implantology, offering advanced treatments, skilled specialists, and exceptional value. AZPO Health connects you with premier dental clinics and comprehensive support for a successful and transformative dental implant journey.",
//       path: "implantology",
//       categoryKey: "dental-treatments",
//     },
//     {
//       img: destigeneral,
//       title: isRu ? "" : "General Dentistry",
//       description: isRu
//         ? ""
//         : "Prioritizing your oral health and seeking routine dental care without high costs? Turkey has become an increasingly popular destination for general dentistry, offering accessible and quality treatments. Discover how AZPO Health connects you with experienced dentists and comfortable, comprehensive care packages to maintain a healthy and confident smile.",
//       path: "general-dentistry",
//       categoryKey: "dental-treatments",
//     },
//     {
//       img: cosmeticdentistry,
//       title: isRu ? "" : "Cosmetic Dentistry",
//       description: isRu
//         ? ""
//         : "Dreaming of a dazzling smile makeover to boost your confidence? Turkey has become a global hotspot for cosmetic dentistry, offering world-class treatments, skilled specialists, and exceptional affordability. Discover how AZPO Health connects you with leading cosmetic dentists and all-inclusive packages to achieve the smile you've always desired.",
//       path: "cosmetic-dentistry",
//       categoryKey: "dental-treatments",
//     },
//     {
//       img: restoryDenstiry,
//       title: isRu ? "" : "Restorative Dentistry",
//       description: isRu
//         ? ""
//         : "Experiencing dental problems that impact your smile’s function and appearance? Restorative dentistry in Turkey offers advanced solutions to repair damaged or missing teeth, restoring your oral health and smile confidence. Discover how AZPO Health connects you with leading restorative dentists and all-inclusive packages for expert care and lasting results.",
//       path: "restorative-dentistry",
//       categoryKey: "dental-treatments",
//     },
//     {
//       img: periodontics,
//       title: isRu ? "" : "Periodontics",
//       description: isRu
//         ? ""
//         : "Concerned about gum disease or seeking specialized gum treatment for a healthier smile? Periodontics in Turkey offers access to highly skilled specialists and advanced treatments at affordable prices. Discover how AZPO Health connects you with leading periodontists and all-inclusive packages to restore your gum health and achieve a confident, lasting smile.",
//       path: "periodontics",
//       categoryKey: "dental-treatments",
//     },
//     {
//       img: orthodontics,
//       title: isRu ? "" : "Orthodontics",
//       description: isRu
//         ? ""
//         : "Achieve a healthier, more confident smile with advanced orthodontic care. Whether it’s braces, clear aligners, or retainers, we offer personalized solutions for all ages. Say goodbye to misaligned teeth and bite issues—our treatments bring precision, comfort, and long-term results. Your perfect smile starts here! 🦷",
//       path: "orthodontics",
//       categoryKey: "dental-treatments",
//     },
//     {
//       img: oralsurgery,
//       title: isRu ? "" : "Oral Surgery",
//       description: isRu
//         ? ""
//         : "From tooth extractions to jaw corrections, our expert oral surgery services restore your smile’s health and function with precision and care. Whether it’s wisdom teeth removal, dental implants, or corrective procedures, we ensure a safe, comfortable, and effective experience. Your oral health is in expert hands!",
//       path: "oral-surgery",
//       categoryKey: "dental-treatments",
//     },
//     // eye
//     {
//       img: eyedesis,
//       title: isRu ? "" : "Eye disease treatments",
//       description: isRu
//         ? ""
//         : "Don't let eye diseases impact your life and well-being any longer. At AZPO Health in Turkey, we offer a range of advanced treatments and expert care to help you see clearly again. Discover how our specialized services can provide the vision solutions you've been searching for.",
//       path: "eye-disease-treatments",
//       categoryKey: "eye-surgery",
//     },
//     {
//       img: surgical,
//       title: isRu ? "" : "Surgical implant procedures",
//       description: isRu
//         ? ""
//         : "Are you tired of struggling with vision problems, from cataracts and refractive errors to the limitations of glasses and contact lenses? Discover how advanced eye surgical & implant procedures in Turkey offered through AZPO Health can provide you with a renewed sense of sight and a brighter future.",
//       path: "surgical-implant-procedures",
//       categoryKey: "eye-surgery",
//     },
//     // fertility
//     {
//       img: asissfertility,
//       title: isRu ? "" : "Fertility assisted reproduction",
//       description: isRu
//         ? ""
//         : "Are you dreaming of starting or expanding your family but facing challenges? Discover how AZPO Health in Turkey can be your partner, offering a ray of hope with world-class fertility treatments and compassionate care, guiding you towards the joyous journey of parenthood.",
//       path: "fertility-assisted-reproduction",
//       categoryKey: "fertility",
//     },
//     // stem cell
//     {
//       img: malesecual,
//       title: isRu ? "" : "Male sexual therapy",
//       description: isRu
//         ? ""
//         : "Are you seeking innovative solutions to revitalize your sexual health and reproductive capabilities? Discover the potential of male sexual & reproductive health stem cell therapy in Turkey, offering cutting-edge treatments designed to address various concerns and improve your quality of life.",
//       path: "male-sexual-therapy",
//       categoryKey: "stem-cell",
//     },
//     {
//       img: theregemed,
//       title: isRu ? "" : "Therapy regenerative medicine",
//       description: isRu
//         ? ""
//         : "Looking for advanced, innovative treatments to restore health and vitality? Stem Cell Therapy & Regenerative Medicine in Turkey offer groundbreaking solutions for tissue repair, disease management, and overall well-being. With cutting-edge medical advancements and expert specialists, these treatments provide new hope for lasting recovery and improved quality of life.",
//       path: "therapy-regenerative-medicine",
//       categoryKey: "stem-cell",
//     },
//   ];
//   return treatments;
// };

export const allSubCategory = (locale: string) => {
  const isRu = locale === "ru";
  const treatments = [
    {
      img: hair,
      title: isRu ? "Пересадка волос" : "Hair Transplantation",
      description: isRu
        ? "Устали от редеющих волос или отступающей линии роста волос? Откройте для себя доступные решения мирового класса по пересадке волос в AZPO Health в Стамбуле, Турция. Верните себе молодую внешность и уверенность с нашей опытной медицинской командой и передовыми технологиями."
        : "Tired of thinning hair or a receding hairline? Discover affordable, world-class hair transplant solutions at AZPO Health in Istanbul, Turkey. Regain your youthful appearance and confidence with our experienced medical team and state-of-the-art techniques.",
      categoryKey: "aesthetic",
      path: "hair-transplantation",
    },
    {
      img: fecial,
      title: isRu ? "Эстетическая хирургия лица" : "Facial Aesthetic Surgery",
      description: isRu
        ? "С возрастом или из-за генетики теряете уверенность? Откройте для себя эстетические процедуры мирового класса для лица в Стамбуле, Анталье и Алании для естественного, обновленного образа! Свяжитесь с AZPO Health сегодня для бесплатной консультации и раскройте лучшую версию себя!"
        : "Is aging or genetics affecting your confidence? Discover world-class facial aesthetic treatments in Istanbul, Antalya, and Alanya for a natural, refreshed you! Contact AZPO Health today for a free consultation and reveal your best self!",
      categoryKey: "aesthetic",
      path: "facial-aesthetic-surgery",
    },
    {
      img: body,
      title: isRu ? "Контурная пластика тела" : "Body contouring",
      description: isRu
        ? "Достигните тела своей мечты с помощью передовых процедур контурной пластики тела в Турции. Испытайте медицинское обслуживание мирового уровня, индивидуальные планы лечения и потрясающие результаты."
        : "Achieve the body of your dreams with advanced body contouring procedures in Turkey. Experience world-class medical care, personalized treatment plans, and stunning results.",
      categoryKey: "aesthetic",
      path: "body-contouring",
    },
    {
      img: breastCategory,
      title: isRu ? "Хирургия груди" : "Breast Surgery",
      description: isRu
        ? "Рассматриваете хирургию груди? Раскройте силуэт своей мечты и повысьте самооценку с помощью хирургии груди мирового класса в Турции. AZPO Health предлагает опытных хирургов, роскошный уход и доступные пакеты. Исследуйте свои варианты увеличения, уменьшения и подтяжки груди в ведущем направлении медицинского туризма."
        : "Considering breast surgery? Unlock your dream silhouette and boost your self-esteem with world-class breast surgery in Turkey. AZPO Health offers expert surgeons, luxurious care, and affordable packages. Explore your options for breast augmentation, reduction, and lift in a premier medical tourism destination.",
      categoryKey: "aesthetic",
      path: "breast-surgery",
    },
    {
      img: genital,
      title: isRu ? "Эстетическая гинекология" : "Genital Aesthetic",
      description: isRu
        ? "Рассматриваете эстетическую гинекологическую хирургию для решения личных проблем или улучшения интимного благополучия? Турция стала ведущим направлением для этих специализированных процедур, предлагая конфиденциальный и высококачественный уход. Узнайте, как AZPO Health связывает вас с опытными хирургами и комплексными пакетами для комфортного и трансформирующего опыта."
        : "Considering genital aesthetic surgery to address personal concerns or enhance your intimate well-being? Turkey has become a leading destination for these specialized procedures, offering discreet, high-quality care. Discover how AZPO Health connects you with expert surgeons and all-inclusive packages for a comfortable and transformative experience.",
      categoryKey: "aesthetic",
      path: "genital-aesthetic",
    },
    {
      img: healthlogo,
      title: isRu ? "Бариатрическая хирургия" : "Bariatric Surgery",
      description: isRu
        ? "Сталкиваетесь с ежедневными трудностями из-за лишнего веса, такими как снижение подвижности, усталость или связанные с этим состояния здоровья? Многие люди чувствуют себя неуверенно или ограниченно из-за своего веса, что влияет на качество их жизни. Бариатрическая хирургия предлагает мощный инструмент, чтобы помочь людям восстановить контроль над своим здоровьем и открыть более активное, полноценное будущее."
        : "Are you facing the daily struggles associated with excess weight, such as reduced mobility, fatigue, or related health conditions? Many people feel self-conscious or limited by their weight, impacting their quality of life. Finding a treatment that offers significant, long-term weight loss can seem daunting. Bariatric surgery offers a powerful tool to help individuals regain control of their health and unlock a more active, fulfilling future.",
      categoryKey: "aesthetic",
      path: "bariatric-surgery",
    },
    // dental
    {
      img: implantology,
      title: isRu ? "Имплантология" : "Implantology",
      description: isRu
        ? "Готовы заменить отсутствующие зубы и вернуть уверенность в полной, здоровой улыбке? Турция стала мировым лидером в области зубной имплантологии, предлагая передовые методы лечения, квалифицированных специалистов и исключительную ценность. AZPO Health связывает вас с ведущими стоматологическими клиниками и комплексной поддержкой для успешного и трансформирующего путешествия по установке зубных имплантатов."
        : "Ready to replace missing teeth and rediscover the confidence of a complete, healthy smile? Turkey has become a global leader in dental implantology, offering advanced treatments, skilled specialists, and exceptional value. AZPO Health connects you with premier dental clinics and comprehensive support for a successful and transformative dental implant journey.",
      path: "implantology",
      categoryKey: "dental-treatments",
    },
    {
      img: destigeneral,
      title: isRu ? "Общая стоматология" : "General Dentistry",
      description: isRu
        ? "Стремитесь поддерживать здоровье полости рта и ищете регулярный стоматологический уход без высоких затрат? Турция стала популярным направлением для общей стоматологии, предлагая доступные и качественные процедуры. Узнайте, как AZPO Health связывает вас с опытными стоматологами и комфортными, комплексными пакетами ухода для поддержания здоровой и уверенной улыбки."
        : "Prioritizing your oral health and seeking routine dental care without high costs? Turkey has become an increasingly popular destination for general dentistry, offering accessible and quality treatments. Discover how AZPO Health connects you with experienced dentists and comfortable, comprehensive care packages to maintain a healthy and confident smile.",
      path: "general-dentistry",
      categoryKey: "dental-treatments",
    },
    {
      img: cosmeticdentistry,
      title: isRu ? "Эстетическая стоматология" : "Cosmetic Dentistry",
      description: isRu
        ? "Мечтаете о потрясающей улыбке, чтобы повысить уверенность? Турция стала мировым центром эстетической стоматологии, предлагая процедуры мирового класса, квалифицированных специалистов и исключительную доступность. Узнайте, как AZPO Health связывает вас с ведущими косметическими стоматологами и комплексными пакетами для достижения улыбки, о которой вы всегда мечтали."
        : "Dreaming of a dazzling smile makeover to boost your confidence? Turkey has become a global hotspot for cosmetic dentistry, offering world-class treatments, skilled specialists, and exceptional affordability. Discover how AZPO Health connects you with leading cosmetic dentists and all-inclusive packages to achieve the smile you've always desired.",
      path: "cosmetic-dentistry",
      categoryKey: "dental-treatments",
    },
    {
      img: restoryDenstiry,
      title: isRu ? "Реставрационная стоматология" : "Restorative Dentistry",
      description: isRu
        ? "Сталкиваетесь с зубными проблемами, которые влияют на функцию и внешний вид вашей улыбки? Реставрационная стоматология в Турции предлагает передовые решения для восстановления поврежденных или отсутствующих зубов, возвращая здоровье полости рта и уверенность в улыбке. Узнайте, как AZPO Health связывает вас с ведущими реставрационными стоматологами и комплексными пакетами для экспертного ухода и долговременных результатов."
        : "Experiencing dental problems that impact your smile’s function and appearance? Restorative dentistry in Turkey offers advanced solutions to repair damaged or missing teeth, restoring your oral health and smile confidence. Discover how AZPO Health connects you with leading restorative dentists and all-inclusive packages for expert care and lasting results.",
      path: "restorative-dentistry",
      categoryKey: "dental-treatments",
    },
    {
      img: periodontics,
      title: isRu ? "Пародонтология" : "Periodontics",
      description: isRu
        ? "Обеспокоены заболеваниями десен или ищете специализированное лечение десен для более здоровой улыбки? Пародонтология в Турции предоставляет доступ к высококвалифицированным специалистам и передовым методам лечения по доступным ценам. Узнайте, как AZPO Health связывает вас с ведущими пародонтологами и комплексными пакетами для восстановления здоровья десен и достижения уверенной, долговременной улыбки."
        : "Concerned about gum disease or seeking specialized gum treatment for a healthier smile? Periodontics in Turkey offers access to highly skilled specialists and advanced treatments at affordable prices. Discover how AZPO Health connects you with leading periodontists and all-inclusive packages to restore your gum health and achieve a confident, lasting smile.",
      path: "periodontics",
      categoryKey: "dental-treatments",
    },
    {
      img: orthodontics,
      title: isRu ? "Ортодонтия" : "Orthodontics",
      description: isRu
        ? "Достигните более здоровой и уверенной улыбки с помощью передовой ортодонтической помощи. Будь то брекеты, прозрачные элайнеры или ретейнеры, мы предлагаем персонализированные решения для всех возрастов. Попрощайтесь с неровными зубами и проблемами прикуса — наши процедуры обеспечивают точность, комфорт и долгосрочные результаты. Ваша идеальная улыбка начинается здесь! 🦷"
        : "Achieve a healthier, more confident smile with advanced orthodontic care. Whether it’s braces, clear aligners, or retainers, we offer personalized solutions for all ages. Say goodbye to misaligned teeth and bite issues—our treatments bring precision, comfort, and long-term results. Your perfect smile starts here! 🦷",
      path: "orthodontics",
      categoryKey: "dental-treatments",
    },
    {
      img: oralsurgery,
      title: isRu ? "Челюстно-лицевая хирургия" : "Oral Surgery",
      description: isRu
        ? "От удаления зубов до коррекции челюсти, наши экспертные услуги челюстно-лицевой хирургии восстанавливают здоровье и функцию вашей улыбки с точностью и заботой. Будь то удаление зубов мудрости, зубные имплантаты или корректирующие процедуры, мы обеспечиваем безопасный, комфортный и эффективный опыт. Ваше здоровье полости рта в надежных руках!"
        : "From tooth extractions to jaw corrections, our expert oral surgery services restore your smile’s health and function with precision and care. Whether it’s wisdom teeth removal, dental implants, or corrective procedures, we ensure a safe, comfortable, and effective experience. Your oral health is in expert hands!",
      path: "oral-surgery",
      categoryKey: "dental-treatments",
    },
    // eye
    {
      img: eyedesis,
      title: isRu ? "Лечение заболеваний глаз" : "Eye disease treatments",
      description: isRu
        ? "Не позволяйте заболеваниям глаз влиять на вашу жизнь и благополучие. В AZPO Health в Турции мы предлагаем ряд передовых методов лечения и экспертный уход, чтобы помочь вам снова видеть ясно. Узнайте, как наши специализированные услуги могут предоставить решения для вашего зрения, которые вы искали."
        : "Don't let eye diseases impact your life and well-being any longer. At AZPO Health in Turkey, we offer a range of advanced treatments and expert care to help you see clearly again. Discover how our specialized services can provide the vision solutions you've been searching for.",
      path: "eye-disease-treatments",
      categoryKey: "eye-surgery",
    },
    {
      img: surgical,
      title: isRu
        ? "Хирургические имплантационные процедуры"
        : "Surgical implant procedures",
      description: isRu
        ? "Устали от проблем со зрением, от катаракты и рефракционных ошибок до ограничений, связанных с очками и контактными линзами? Узнайте, как передовые хирургические процедуры для глаз и имплантации в Турции, предлагаемые через AZPO Health, могут предоставить вам обновленное чувство зрения и более светлое будущее."
        : "Are you tired of struggling with vision problems, from cataracts and refractive errors to the limitations of glasses and contact lenses? Discover how advanced eye surgical & implant procedures in Turkey offered through AZPO Health can provide you with a renewed sense of sight and a brighter future.",
      path: "surgical-implant-procedures",
      categoryKey: "eye-surgery",
    },
    // fertility
    {
      img: asissfertility,
      title: isRu
        ? "Вспомогательная репродукция"
        : "Fertility assisted reproduction",
      description: isRu
        ? "Мечтаете о создании или расширении семьи, но сталкиваетесь с трудностями? Узнайте, как AZPO Health в Турции может стать вашим партнером, предлагая луч надежды с помощью репродуктивных процедур мирового класса и заботливого ухода, направляя вас к радостному пути родительства."
        : "Are you dreaming of starting or expanding your family but facing challenges? Discover how AZPO Health in Turkey can be your partner, offering a ray of hope with world-class fertility treatments and compassionate care, guiding you towards the joyous journey of parenthood.",
      path: "fertility-assisted-reproduction",
      categoryKey: "fertility",
    },
    // stem cell
    {
      img: malesecual,
      title: isRu
        ? "Терапия мужского сексуального здоровья"
        : "Male sexual therapy",
      description: isRu
        ? "Ищете инновационные решения для восстановления сексуального здоровья и репродуктивных возможностей? Откройте для себя потенциал терапии стволовыми клетками для мужского сексуального и репродуктивного здоровья в Турции, предлагающей передовые методы лечения, направленные на решение различных проблем и улучшение качества жизни."
        : "Are you seeking innovative solutions to revitalize your sexual health and reproductive capabilities? Discover the potential of male sexual & reproductive health stem cell therapy in Turkey, offering cutting-edge treatments designed to address various concerns and improve your quality of life.",
      path: "male-sexual-therapy",
      categoryKey: "stem-cell",
    },
    {
      img: theregemed,
      title: isRu ? "Регенеративная медицина" : "Therapy regenerative medicine",
      description: isRu
        ? "Ищете передовые, инновационные методы лечения для восстановления здоровья и жизненной силы? Терапия стволовыми клетками и регенеративная медицина в Турции предлагают революционные решения для восстановления тканей, управления заболеваниями и общего благополучия. С передовыми медицинскими достижениями и экспертными специалистами эти процедуры дают новую надежду на длительное восстановление и улучшение качества жизни."
        : "Looking for advanced, innovative treatments to restore health and vitality? Stem Cell Therapy & Regenerative Medicine in Turkey offer groundbreaking solutions for tissue repair, disease management, and overall well-being. With cutting-edge medical advancements and expert specialists, these treatments provide new hope for lasting recovery and improved quality of life.",
      path: "therapy-regenerative-medicine",
      categoryKey: "stem-cell",
    },
  ];
  return treatments;
};
export const allSubNestedCategory = (locale: string) => {
  const isRu = locale === "ru";
  const treatments = [
    // bariatric-surgery
    {
      img: healthlogo,
      title: isRu
        ? "Внутрижелудочный баллон в Турции"
        : "Intragastric Balloon in Turkey",
      description: isRu
        ? "Похудение может быть сложным и часто обескураживающим путешествием. Если вы пробовали различные диеты и программы упражнений без длительного успеха, но идея постоянной бариатрической хирургии кажется слишком большим шагом, вы можете искать менее инвазивный вариант. Чувство разочарования из-за упрямого веса и его влияния на ваше здоровье и уверенность — это нормально, но существуют эффективные нехирургические решения."
        : "Losing weight can be a difficult and often discouraging journey. If you've tried various diets and exercise programs without lasting success, but the idea of permanent weight loss surgery feels like too big a step, you might be looking for a less invasive option. Feeling frustrated by stubborn weight and its impact on your health and confidence is common, but there are effective, non-surgical solutions available.",
      path: "intragastric-balloon",
      categoryKey: "aesthetic",
      subCategoryKey: "bariatric-surgery",
    },
    {
      img: healthlogo,
      title: isRu
        ? "Билиопанкреатическое шунтирование с дуоденальным переключением (BPD/DS) в Турции"
        : "Biliopancreatic Diversion with Duodenal Switch (BPD/DS) in Turkey",
      description: isRu
        ? "Для людей, борющихся с тяжелым или морбидным ожирением (часто определяемым как ИМТ 50 или выше), путь может быть невероятно сложным. Стандартные методы похудения могли оказаться неэффективными, и даже другие бариатрические операции могут не предложить глубокую, длительную потерю веса, необходимую для значительного улучшения здоровья и восстановления качества жизни. Если вы оказались в такой ситуации, чувствуя себя подавленным из-за осложнений, связанных с экстремальным ожирением, и ищете самое мощное хирургическое решение, билиопанкреатическое шунтирование с дуоденальным переключением (BPD/DS) может быть ответом."
        : "For individuals grappling with severe or morbid obesity (often defined as a BMI of 50 or higher), the journey can be incredibly challenging. Standard weight loss methods may have proven ineffective, and even other bariatric surgeries might not offer the profound, lasting weight loss needed to significantly improve health and reclaim quality of life. If you find yourself in this situation, feeling overwhelmed by the health complications of extreme obesity and seeking the most potent surgical solution available, Biliopancreatic Diversion with Duodenal Switch (BPD/DS) could be the answer.",
      path: "bpd-ds",
      categoryKey: "aesthetic",
      subCategoryKey: "bariatric-surgery",
    },
    {
      img: healthlogo,
      title: isRu
        ? "Регулируемое желудочное бандажирование в Турции"
        : "Adjustable Gastric Banding in Turkey",
      description: isRu
        ? "Изучаете варианты бариатрической хирургии, но сомневаетесь в постоянных изменениях вашего тела? Возможно, вы ищете менее инвазивный подход для борьбы с ожирением и связанными с ним проблемами здоровья. Если диета и упражнения не дали долгосрочных результатов, и вы предпочитаете процедуру, которую можно регулировать или даже отменить, регулируемое желудочное бандажирование может быть правильным выбором для вас."
        : "Are you exploring weight loss surgery options but feel hesitant about permanent changes to your body? Perhaps you're looking for a less invasive approach to help manage obesity and its related health challenges. If diet and exercise haven't provided the lasting results you need, and you prefer a procedure that can be adjusted or even reversed, Adjustable Gastric Banding might be the right path for you.",
      path: "adjustable-gastric-banding",
      categoryKey: "aesthetic",
      subCategoryKey: "bariatric-surgery",
    },
    {
      img: healthlogo,
      title: isRu
        ? "Гастрошунтирование по Ру в Турции"
        : "Roux-en-Y Gastric Bypass Surgery",
      description: isRu
        ? "Жизнь с тяжелым ожирением может ощущаться как постоянная борьба. Возможно, вы пробовали бесчисленные диеты и планы упражнений, но вес возвращался, оставляя вас разочарованным и безнадежным. Возможно, вы сталкиваетесь с серьезными проблемами со здоровьем, такими как диабет 2 типа, высокое давление или апноэ во сне, которые ограничивают ваши повседневные активности и влияют на ваше общее благополучие. Мы понимаем ваши трудности и связываем вас с самыми авторитетными клиниками Турции и высококвалифицированными бариатрическими хирургами, специализирующимися на трансформирующих процедурах похудения, включая известное гастрошунтирование по Ру."
        : "Living with severe obesity can feel like a constant battle. Perhaps you've tried countless diets and exercise plans, only to find the weight returning, leaving you feeling frustrated and hopeless. Maybe you're facing serious health problems like type 2 diabetes, high blood pressure, or sleep apnea, which limit your daily activities and impact your overall well-being.  We understand the challenges you face, and we connect you with Turkey's most reputable clinics and highly experienced bariatric surgeons specializing in transformative weight-loss procedures, including the renowned Roux-en-Y Gastric Bypass.",
      path: "roux-gastric-bypass",
      categoryKey: "aesthetic",
      subCategoryKey: "bariatric-surgery",
    },
    {
      img: healthlogo,
      title: isRu
        ? "Рукавная гастрэктомия в Турции"
        : "Gastric Sleeve Surgery in Turkey",
      description: isRu
        ? "Трансформационное путешествие к более здоровому вам с помощью рукавной гастрэктомии, ведущей бариатрической процедуры, предлагаемой AZPO Health в Турции. Если вы боролись с ожирением и связанными с ним проблемами здоровья, рукавная гастрэктомия в Турции предоставляет эффективное, безопасное и доступное решение. AZPO Health сотрудничает с лучшими, международно аккредитованными клиниками и высококвалифицированными бариатрическими хирургами по всей Турции, гарантируя, что вы получите медицинский уход мирового уровня, адаптированный к вашим потребностям."
        : "A transformative journey towards a healthier you with gastric sleeve surgery, a leading bariatric procedure offered by AZPO Health in Turkey. If you've struggled with obesity and related health issues, gastric sleeve surgery in Turkey provides an effective, safe, and affordable solution. AZPO Health partners with the best, internationally accredited clinics and highly experienced bariatric surgeons across Turkey's top destinations, ensuring you receive world-class care tailored to your needs.",
      path: "gastric-sleeve-surgery",
      categoryKey: "aesthetic",
      subCategoryKey: "bariatric-surgery",
    },

    // hair-transplantation
    {
      title: isRu ? "FUE пересадка волос" : "FUE Hair Transplantation",
      img: FUE,
      description: isRu
        ? "Устали от редеющих волос? Откройте для себя решения по пересадке волос FUE мирового класса в AZPO Health в Стамбуле, Анталье и Аланьяе. Верните молодую внешность и уверенность с нашей опытной командой и передовыми технологиями."
        : "Tired of thinning hair? Discover world-class FUE hair transplant solutions at AZPO Health in Istanbul, Antalya, and Alanya. Regain your youthful look and confidence with our experienced team and advanced techniques.",
      path: "fue-hair-transplant",
      categoryKey: "aesthetic",
      subCategoryKey: "hair-transplantation",
    },
    {
      title: isRu ? "DHI пересадка волос" : "DHI Hair Transplant",
      img: DHI,
      description: isRu
        ? "Страдаете от выпадения волос? Откройте инновационную технику DHI в Турции для естественного, безболезненного и постоянного решения. AZPO Health предлагает экспертизу, комплексные пакеты и потрясающие результаты в Стамбуле, Анталья и Алянья."
        : "Suffering from hair loss? Discover the innovative DHI technique in Turkey for a natural-looking, painless, and permanent solution. AZPO Health offers expertise, comprehensive packages, and stunning results in Istanbul, Antalya, and Alanya.",
      categoryKey: "aesthetic",
      path: "dhi-hair-transplant",
      subCategoryKey: "hair-transplantation",
    },
    {
      img: prp,
      title: isRu ? "PRP-терапия волос" : "PRP Hair Treatment",
      description: isRu
        ? "Откройте для себя преимущества PRP-пересадки волос в клиниках AZPO Health в Стамбуле, Анталье и Алании. Стимулируйте волосяные фолликулы, уменьшайте выпадение волос и достигайте более густой, здоровой шевелюры."
        : "Discover the benefits of PRP hair transplantation at AZPO Health clinics in Istanbul, Antalya, and Alanya. Stimulate hair follicles, reduce hair loss, and achieve a fuller, healthier head of hair.",
      path: "prp-hair-treatment",
      categoryKey: "aesthetic",
      subCategoryKey: "hair-transplantation",
    },
    {
      img: Mesotherapy,
      title: isRu ? "Мезотерапия для волос" : "Mesotherapy Hair Treatment ",
      description: isRu
        ? "Редеющие волосы снижают вашу уверенность? Откройте для себя трансформирующую силу мезотерапии для волос в AZPO Health в Турции. Наши экспертные процедуры в Стамбуле, Анталье и Алании стимулируют рост волос, питают кожу головы и восстанавливают естественную жизненную силу ваших волос. Свяжитесь с нами сегодня для бесплатной консультации и верните себе молодую внешность!"
        : "Is thinning hair affecting your confidence? Discover the transformative power of hair mesotherapy at AZPO Health in Turkey. Our expert treatments in Istanbul, Antalya, and Alanya stimulate hair growth, nourish your scalp, and restore your hair's natural vitality. Contact us today for a free consultation and reclaim your youthful appearance!",
      path: "mesotherapy-hair",
      categoryKey: "aesthetic",
      subCategoryKey: "hair-transplantation",
    },
    {
      img: Beard,
      title: isRu ? "Пересадка бороды" : "Beard Transplant",
      description: isRu
        ? "Мечтаете о более густой, четко очерченной бороде? Откройте для себя трансформирующую силу пересадки бороды в Турции. С передовыми техниками и опытными специалистами AZPO Health предлагает естественные, постоянные решения, чтобы помочь вам достичь идеальной бороды. Свяжитесь с нами для бесплатной консультации и раскройте потенциал вашей бороды!"
        : "Dreaming of a fuller, more defined beard? Discover the transformative power of beard transplants in Turkey. With advanced techniques and experienced specialists, AZPO Health offers natural-looking, permanent solutions to help you achieve your ideal beard. Contact us for a free consultation and unlock your beard potential!",
      path: "beard-transplant",
      categoryKey: "aesthetic",
      subCategoryKey: "hair-transplantation",
    },
    {
      img: Moustache,
      title: isRu ? "Пересадка усов" : "Moustache Transplant",
      description: isRu
        ? "Мечтаете о более густых и четко очерченных усах? Откройте для себя естественные и постоянные решения по пересадке усов в AZPO Health в Турции!"
        : "Dream of a fuller, more defined moustache? Discover natural and permanent moustache transplant solutions at AZPO Health in Turkey!",
      path: "moustache-transplant",
      categoryKey: "aesthetic",
      subCategoryKey: "hair-transplantation",
    },
    {
      img: Eyebrow,
      title: isRu ? "Пересадка бровей" : "Eyebrow Transplant",
      description: isRu
        ? "Достигайте идеально сформированных, естественно выглядящих бровей с экспертными процедурами пересадки бровей AZPO Health в Стамбуле, Анталье и Алании."
        : "Achieve perfectly shaped, natural-looking eyebrows with AZPO Health's expert eyebrow transplant procedures in Istanbul, Antalya, and Alanya.",
      path: "eyebrow-transplant",
      categoryKey: "aesthetic",
      subCategoryKey: "hair-transplantation",
    },
    // facial
    {
      img: rhinoplasty,
      title: isRu
        ? "Ринопластика: изменение формы носа"
        : "Rhinoplasty Nose Reshaping",
      description: isRu
        ? "Чувствуете себя неуверенно из-за внешнего вида вашего носа или сталкиваетесь с проблемами дыхания? В 2025 году AZPO Health в Турции предлагает естественно выглядящую ринопластику с постоянными результатами, помогая вам вернуть уверенность."
        : "Feeling self-conscious about your nose's appearance or struggling with breathing issues? In 2025, AZPO Health in Turkey offers natural-looking rhinoplasty with permanent results, helping you regain confidence.",
      path: "rhinoplasty-nose-reshaping",
      categoryKey: "aesthetic",
      subCategoryKey: "facial-aesthetic-surgery",
    },
    {
      img: double,
      title: isRu
        ? "Липосакция двойного подбородка"
        : "Double chin liposuction",
      description: isRu
        ? "Устали видеть упрямый двойной подбородок на фотографиях или в зеркале? Неважно, как много вы сидите на диете или занимаетесь спортом, этот лишний жир под подбородком просто не уходит? Это раздражает — но есть решение. С липосакцией двойного подбородка в Турции вы можете добиться четкой, скульптурной линии челюсти и вернуть молодую, уверенную внешность. AZPO Health связывает вас с ведущими косметическими специалистами, которые используют передовые, минимально инвазивные техники для гладких, естественно выглядящих результатов."
        : "Tired of seeing a stubborn double chin in photos or the mirror? No matter how much you diet or exercise, that extra fat under your chin just won’t go away? It’s frustrating—but there’s a solution. With Double Chin Liposuction in Turkey, you can achieve a sleek, sculpted jawline and bring back your youthful, confident look. AZPO Health connects you with top cosmetic specialists who use advanced, minimally invasive techniques for smooth, natural-looking results.",
      path: "double-chin-liposuction",
      categoryKey: "aesthetic",
      subCategoryKey: "facial-aesthetic-surgery",
    },
    {
      img: otoplasty,
      title: isRu ? "Отопластика" : "Otoplasty",
      description: isRu
        ? "Обеспокоены выступающими или деформированными ушами? AZPO Health в Турции предлагает операцию отопластики для изменения формы ушей, повышения уверенности в себе и обеспечения естественных, долговременных результатов."
        : "Are you concerned about prominent or misshapen ears? AZPO Health in Turkey offers otoplasty surgery to reshape your ears, boost your self-esteem, and provide natural, long-lasting results.",
      path: "otoplasty",
      categoryKey: "aesthetic",
      subCategoryKey: "facial-aesthetic-surgery",
    },
    {
      img: ble,
      title: isRu
        ? "Блефаропластика верхних и нижних век"
        : "Blepharoplasty upper lower",
      description: isRu
        ? "Устали выглядеть уставшим? Опущенные веки и мешки под глазами могут добавлять годы к вашему лицу, заставляя вас выглядеть измученным или старше, чем вы чувствуете. С экспертной блефаропластикой (операцией на веках) в Турции вы сможете достичь обновленного, молодого вида в 2025 году. В AZPO Health мы соединяем вас с ведущими хирургами, специализирующимися на естественно выглядящем омоложении век — помогая уменьшить отечность, подтянуть обвисшую кожу и восстановить более яркое, бодрое выражение лица."
        : "Tired of looking tired? Sagging eyelids and under-eye bags can add years to your face, making you look exhausted or older than you feel. With expert Blepharoplasty (eyelid surgery) in Turkey, you can achieve a refreshed, youthful appearance in 2025. At AZPO Health, we connect you with top surgeons specializing in natural-looking eyelid rejuvenation—helping you reduce puffiness, lift drooping skin, and restore a brighter, more awake expression.",
      path: "blepharoplasty-upper-lower",
      categoryKey: "aesthetic",
      subCategoryKey: "facial-aesthetic-surgery",
    },
    {
      img: brow,
      title: isRu ? "Подтяжка бровей" : "Brow Lift",
      description: isRu
        ? "Опущенные брови, морщины на лбу или нависшие веки делают вас уставшим, старше или менее выразительным? Не позволяйте старению определять ваш внешний вид — обновите и подтяните свой взгляд с помощью экспертной операции подтяжки бровей в Турции. В AZPO Health мы соединяем вас с ведущими хирургами, специализирующимися на подтяжке бровей, обеспечивая естественно выглядящие результаты."
        : "Are sagging brows, forehead wrinkles, or hooded eyelids making you look tired, older, or less expressive? Don’t let aging define you—refresh and lift your look with expert Brow Lift Surgery in Turkey. At AZPO Health, we connect you with top-tier surgeons who specialize in Brow Lift Surgery and giving you natural-looking results.",
      path: "brow-lift",
      categoryKey: "aesthetic",
      subCategoryKey: "facial-aesthetic-surgery",
    },
    {
      img: neck,
      title: isRu ? "Подтяжка шеи" : "Neck Lift",
      description: isRu
        ? "Если обвисшая кожа, морщины или дряблая линия шеи заставляют вас чувствовать себя старше, чем вы есть, пришло время для перемен. Подтяжка шеи может восстановить более упругий, молодой контур, возвращая уверенность, которая приходит с четко очерченной линией челюсти. В AZPO Health мы соединяем вас с ведущими хирургами в Турции, которые специализируются на естественно выглядящих результатах, обеспечивая гладкий, обновленный внешний вид."
        : "If sagging skin, wrinkles, or a loose neckline is making you feel older than you are, it’s time for a change. A neck lift can restore a firmer, more youthful contour, bringing back the confidence that comes with a well-defined jawline. At AZPO Health, we connect you with top surgeons in Turkey who specialize in natural-looking results, ensuring a smooth, refreshed appearance. Turn back the clock with expert care and lasting transformation.",
      path: "neck-lift",
      categoryKey: "aesthetic",
      subCategoryKey: "facial-aesthetic-surgery",
    },
    // body contouring
    {
      img: liposuction,
      title: isRu ? "Липосакция" : "Liposuction",
      description: isRu
        ? "Боретесь с упрямым жиром? Откройте для себя доступную и эффективную липосакцию в Турции с AZPO Health, чтобы достичь желаемого тела с естественно выглядящими результатами. Чувствуете себя неуверенно из-за локальных жировых отложений? В AZPO Health мы понимаем ваше разочарование. Липосакция в Турции предлагает безопасное и эффективное решение для формирования вашего тела и повышения уверенности. С опытными хирургами и современными учреждениями AZPO Health поможет вам достичь вашей мечты в 2025 году."
        : "Are you struggling with stubborn fat? Discover affordable and effective liposuction in Turkey with AZPO Health, achieving the body you desire with natural-looking results.Feeling self-conscious about localized fat deposits? At AZPO Health, we understand the frustration. Liposuction in Turkey offers a safe and effective solution to sculpt your body and boost your confidence. With experienced surgeons and state-of-the-art facilities, AZPO Health can help you achieve your dream physique in 2025.",
      path: "liposuction",
      categoryKey: "aesthetic",
      subCategoryKey: "body-contouring",
    },
    {
      img: brazilianad,
      title: isRu ? "Бразильская подтяжка ягодиц" : "Brazilian Butt Lift",
      description: isRu
        ? "Мечтаете о более выраженных изгибах? Откройте для себя безопасные и доступные варианты бразильской подтяжки ягодиц (BBL) в Турции с AZPO Health. Достигайте естественно выглядящих результатов и повышайте уверенность."
        : "Dreaming of enhanced curves? Discover safe, affordable Brazilian Butt Lift (BBL) options in Turkey with AZPO Health. Achieve natural-looking results and boost your confidence.",
      path: "brazilian-butt-lift",
      categoryKey: "aesthetic",
      subCategoryKey: "body-contouring",
    },
    {
      img: arm,
      title: isRu ? "Подтяжка рук" : "Arm Lift",
      description: isRu
        ? "Мечтаете о подтянутых, скульптурных руках? Лишняя кожа после похудения или старения может подорвать уверенность. Откройте для себя доступную и трансформирующую операцию по подтяжке рук в Турции с AZPO Health. Восстановите контуры ваших рук и снова наслаждайтесь безрукавными стилями!"
        : "Dreaming of toned, sculpted arms? Excess skin after weight loss or aging can impact confidence. Discover affordable and transformative Armlift surgery in Turkey with AZPO Health. Reclaim your arm contours and embrace sleeveless styles again!",
      path: "arm-lift",
      categoryKey: "aesthetic",
      subCategoryKey: "body-contouring",
    },
    {
      img: thighlift,
      title: isRu ? "Подтяжка бедер" : "Thigh Lift",
      description: isRu
        ? "Устали от обвисшей кожи и упрямого жира на бедрах? AZPO Health связывает вас с ведущими клиниками Турции для трансформирующей операции по подтяжке бедер. Получите красивые рельефные ноги и обновленную уверенность с помощью безопасного, эффективного и доступного решения."
        : "Tired of sagging skin and stubborn thigh fat? AZPO Health connects you with top clinics in Turkey for transformative thigh lift surgery. Achieve beautifully sculpted legs and renewed confidence with a safe, effective, and affordable solution.",
      path: "thigh-lift",
      categoryKey: "aesthetic",
      subCategoryabineKey: "body-contouring",
    },
    {
      img: tummy,
      title: isRu ? "Абдоминопластика" : "Tummy Tuck Abdominoplasty",
      description: isRu
        ? "Лишняя кожа после похудения или беременности заставляет вас чувствовать себя неуверенно? Абдоминопластика (подтяжка живота) с AZPO Health в Турции может удалить эту кожу, подтянуть мышцы живота и восстановить вашу молодую форму. Верните уверенность с более плоским, упругим животом и вступите в новую версию себя."
        : "Is excess skin after weight loss or pregnancy making you feel self-conscious? A tummy tuck (abdominoplasty) with AZPO Health in Turkey can remove that skin, tighten your abdominal muscles, and restore your youthful shape. Regain confidence with a flatter, firmer tummy and step into the new you.",
      path: "tummy-tuck-abdominoplasty",
      categoryKey: "aesthetic",
      subCategoryKey: "body-contouring",
    },
    // breast-surgery
    {
      img: breatSubCategory,
      title: isRu
        ? "Увеличение груди имплантатами"
        : "Breast Augmentation Implants",
      description: isRu
        ? "Желаете более полных, четко очерченных изгибов? Увеличение груди имплантатами с AZPO Health в Турции предлагает трансформирующее путешествие для усиления вашей естественной красоты и повышения уверенности в себе. Узнайте, как наши экспертные хирурги и комплексные пакеты могут воплотить вашу мечту в реальность."
        : "Longing for fuller, more defined curves? Breast augmentation implants with AZPO Health in Turkey offers a transformative journey to enhance your natural beauty and boost your self-confidence. Discover how our expert surgeons and all-inclusive packages can make your dream a reality.",
      categoryKey: "aesthetic",
      path: "breast-augmentation-implants",
      subCategoryKey: "breast-surgery",
    },
    {
      img: breast,
      title: isRu ? "Подтяжка груди (мастопексия)" : "Breast Lift Mastopexy",
      description: isRu
        ? "Чувствуете, что ваши груди потеряли молодую форму и упругость из-за таких факторов, как старение, беременность или колебания веса? Желаете более подтянутый и эстетически привлекательный вид груди? В Турции через обширную сеть AZPO Health вы можете связаться с квалифицированными хирургами, специализирующимися на процедурах подтяжки груди (мастопексии), помогая вам вернуть уверенность и достичь желаемых контуров груди."
        : "Are you feeling that your breasts have lost their youthful shape and firmness due to factors like aging, pregnancy, or weight fluctuations? Do you desire a more lifted and aesthetically pleasing breast appearance? In Turkey, through AZPO Health's extensive network, you can connect with skilled surgeons who specialize in breast lift mastopexy procedures, helping you regain confidence and achieve the breast contours you've been longing for.",
      path: "breast-lift-mastopexy",
      categoryKey: "aesthetic",
      subCategoryKey: "breast-surgery",
    },
    // genital
    // labia
    {
      img: labiaplastyGenital,
      title: isRu ? "Лабиопластика Барби-лук" : "Labiaplasty Barbie Look",
      description: isRu
        ? "Испытываете дискомфорт или неуверенность из-за увеличенных или асимметричных половых губ? Лабиопластика в Турции в AZPO Health предлагает безопасное, эффективное решение для восстановления вашего комфорта, уверенности и благополучия. Под заботой опытных хирургов вы можете достичь желаемых эстетических результатов."
        : "Are you experiencing discomfort or self-consciousness due to enlarged or asymmetrical labia?. Labiaplasty in Turkey at AZPO Health offers a safe, effective solution to restore your comfort, confidence, and well-being. Under the care of experienced surgeons, you can achieve the aesthetic results you desire.",
      categoryKey: "aesthetic",
      path: "labiaplasty-barbie-look",
      subCategoryKey: "genital-aesthetic",
    },
    // panile
    {
      img: penileGenital,
      title: isRu
        ? "Увеличение пениса (фаллопластика)"
        : "Penile Enlargement Phalloplasty",
      description: isRu
        ? "Рассматриваете фаллопластику для увеличения пениса, чтобы повысить уверенность и удовлетворить личные желания? Турция стала уважаемым направлением для мужчин, ищущих эти специализированные процедуры. Узнайте, как AZPO Health может связать вас с опытными хирургами и комплексной поддержкой для конфиденциального и вдохновляющего путешествия."
        : "Considering phalloplasty for penile enlargement to enhance your confidence and address personal desires? Turkey has emerged as a respected destination for men seeking these specialized procedures. Discover how AZPO Health can connect you with experienced surgeons and comprehensive support for a discreet and empowering journey.",
      categoryKey: "aesthetic",
      path: "penile-enlargement-phalloplasty",
      subCategoryKey: "genital-aesthetic",
    },
    // dental
    // Implantology
    {
      img: den64,
      title: isRu
        ? "Зубные имплантаты All-on-6 и All-on-4"
        : "All on 6-4 Dental Implants",
      description: isRu
        ? "Рассматриваете полное восстановление зубной дуги и ищете экономичные решения? Турция стала ведущим направлением для процедур зубных имплантатов All-on-4 и All-on-6, предлагая передовые техники по значительно сниженным ценам. Узнайте, как AZPO Health соединяет вас с экспертными имплантологами и комплексными пакетами, чтобы вернуть вам полную, функциональную и красивую улыбку."
        : "Considering full arch dental restoration and researching cost-effective solutions? Turkey has become a leading destination for 4 Dental Implants and 6 Dental Implants procedures, offering advanced All-on-4 and All-on-6 implant techniques at significantly reduced costs. Discover how AZPO Health connects you with expert implantologists and all-inclusive packages to regain a complete, functional, and beautiful smile.",
      categoryKey: "dental-treatments",
      path: "6-4-dental-implants",
      subCategoryKey: "implantology",
    },
    {
      img: dentalbonding,
      title: isRu ? "Зубная связь" : "Dental Bonding",
      description: isRu
        ? "Стыдитесь мелких недостатков в вашей улыбке, таких как сколы, трещины, пятна или неровные зубы? Представьте себе быстрое, эффективное и доступное решение, которое может преобразить вашу улыбку и повысить уверенность. В Турции квалифицированные стоматологи предлагают высококачественные процедуры зубной связи, и AZPO Health здесь, чтобы связать вас с необходимой экспертизой для более яркой, более уверенной версии вас."
        : "Are you self-conscious about minor imperfections in your smile, such as chipped, cracked, stained, or unevenly spaced teeth? Imagine a quick, effective, and affordable solution that can transform your smile and boost your confidence. In Turkey, skilled dental professionals offer high-quality dental bonding procedures, and AZPO Health is here to connect you with the expertise you need for a brighter, more confident you.",
      categoryKey: "dental-treatments",
      path: "dental-bonding",
      subCategoryKey: "cosmetic-dentistry",
    },
    // dentistry
    {
      img: rootcanal,
      title: isRu ? "Терапия корневых каналов" : "Root Canal Therapy",
      description: isRu
        ? "Стремитесь поддерживать здоровье полости рта и ищете регулярный стоматологический уход без высоких затрат? Турция стала популярным направлением для общей стоматологии, предлагая доступные и качественные процедуры. Узнайте, как AZPO Health связывает вас с опытными стоматологами и комфортными, комплексными пакетами ухода для поддержания здоровой и уверенной улыбки."
        : "Prioritizing your oral health and seeking routine dental care without high costs? Turkey has become an increasingly popular destination for general dentistry, offering accessible and quality treatments. Discover how AZPO Health connects you with experienced dentists and comfortable, comprehensive care packages to maintain a healthy and maintain confident smile.",
      categoryKey: "dental-treatments",
      path: "root-canal-therapy",
      subCategoryKey: "general-dentistry",
    },
    // laminate-veneers
    {
      img: laminateveneers,
      title: isRu ? "Ламинатные виниры" : "Laminate Veneers",
      description: isRu
        ? "Устите от боли в зубах и беспокоитесь о потере вашей естественной улыбки? Откройте, как терапия корневых каналов в Турции может быть вашим доступным и высококачественным решением. AZPO Health предлагает комплексные стоматологические пакеты в Стамбуле, Анталье и Алянье, сочетая экспертное лечение с расслабляющим медицинским туристическим опытом."
        : "Tired of tooth pain and worried about losing your natural smile? Discover how root canal therapy in Turkey can be your affordable and high-quality solution. AZPO Health offers comprehensive dental care packages in Istanbul, Antalya, and Alanya, combining expert treatment with a relaxing medical tourism experience.",
      categoryKey: "dental-treatments",
      path: "laminate-veneers",
      subCategoryKey: "cosmetic-dentistry",
    },
    // Hollywood Smile
    {
      img: hollywood,
      title: isRu
        ? "Голливудская улыбка / Snap-on Smile"
        : "Hollywood smile/Snap on Smile",
      description: isRu
        ? "Получите ослепляющую, идеальную улыбку, о которой вы всегда мечтали, с Голливудской улыбкой. Попрощайтесь с пятнами, сколами и пробелами — наша экспертная косметическая стоматология улучшает ваши зубы с точностью, давая вам идеальный, готовый для фото взгляд. Повысьте вашу уверенность и сделайте каждую улыбку незабываемой!"
        : "Get the dazzling, flawless smile you’ve always dreamed of with Hollywood Smile. Say goodbye to stains, chips, and gaps—our expert cosmetic dentistry enhances your teeth with precision, giving you that perfect, picture-ready look. Boost your confidence and make every smile unforgettable!",
      categoryKey: "dental-treatments",
      path: "hollywood-smile",
      subCategoryKey: "cosmetic-dentistry",
    },
    // teeth whiting
    {
      img: teethwhiting,
      title: isRu ? "Отбеливание зубов" : "Teeth whitening",
      description: isRu
        ? "Устали от боли в зубах и беспокоитесь о потере вашей естественной улыбки? Откройте для себя, как терапия корневых каналов в Турции может быть вашим доступным и высококачественным решением. AZPO Health предлагает комплексные стоматологические пакеты в Стамбуле, Анталье и Алании, сочетая экспертное лечение с расслабляющим медицинским туристическим опытом."
        : "Tired of tooth pain and worried about losing your natural smile? Discover how root canal therapy in Turkey can be your affordable and high-quality solution. AZPO Health offers comprehensive dental care packages in Istanbul, Antalya, and Alanya, combining expert treatment with a relaxing medical tourism experience.",
      categoryKey: "dental-treatments",
      path: "teeth-whitening",
      subCategoryKey: "cosmetic-dentistry",
    },
    // dental-crowns
    {
      img: dentistchoosing,
      title: isRu ? "Зубные коронки" : "Dental Crowns",
      description: isRu
        ? "Устали от боли в зубах и беспокоитесь о потере вашей естественной улыбки? Откройте для себя, как терапия корневых каналов в Турции может быть вашим доступным и высококачественным решением. AZPO Health предлагает комплексные стоматологические пакеты в Стамбуле, Анталье и Алянии, сочетая экспертное лечение с расслабляющим медицинским туристическим опытом."
        : "Tired of tooth pain and worried about losing your natural smile? Discover how root canal therapy in Turkey can be your affordable and high-quality solution. AZPO Health offers comprehensive dental care packages in Istanbul, Antalya, and Alanya, combining expert treatment with a relaxing medical tourism experience.",
      categoryKey: "dental-treatments",
      path: "dental-crowns",
      subCategoryKey: "restorative-dentistry",
    },
    // clear aligners
    {
      img: clearaligners,
      title: isRu
        ? "Прозрачные элайнеры Invisalign"
        : "Clear Aligners Invisalign",
      description: isRu
        ? "Отказывайтесь от металлических брекетов! Прозрачные элайнеры Invisalign предлагают практически невидимый, удобный способ достижения идеально выровненных зубов. Изготовленные на заказ для точного соответствия, эти съемные элайнеры позволяют вам есть, пить и улыбаться с уверенностью — одновременно выравнивая ваши зубы без усилий. Получите улыбку, которую вы заслуживаете, современным способом!"
        : "Ditch the metal braces! Invisalign clear aligners offer a virtually invisible, comfortable way to achieve perfectly aligned teeth. Custom-made for a precise fit, these removable aligners let you eat, drink, and smile with confidence—while straightening your teeth effortlessly. Get the smile you deserve, the modern way!",
      categoryKey: "dental-treatments",
      path: "clear-aligners-invisalign",
      subCategoryKey: "orthodontics",
    },
    // Gum Grafting
    {
      img: gumgrafting,
      title: isRu ? "Пересадка десен" : "Gum Grafting",
      description: isRu
        ? "Отказывайтесь от металлических брекетов! Прозрачные элайнеры Invisalign предлагают практически невидимый, удобный способ достижения идеально ровных зубов. Изготовленные на заказ для точного соответствия, эти съемные элайнеры позволяют вам есть, пить и улыбаться с уверенностью — одновременно выравнивая ваши зубы без усилий. Получите улыбку, которую вы заслуживаете, современным способом!"
        : "Ditch the metal braces! Invisalign clear aligners offer a virtually invisible, comfortable way to achieve perfectly aligned teeth. Custom-made for a precise fit, these removable aligners let you eat, drink, and smile with confidence—while straightening your teeth effortlessly. Get the smile you deserve, the modern way!",
      categoryKey: "dental-treatments",
      path: "gum-grafting",
      subCategoryKey: "periodontics",
    },
    // Laser Gum Treatments
    {
      img: lasergumtreatments,
      title: isRu ? "Лазерные процедуры для десен" : "Laser Gum Treatments",
      description: isRu
        ? "Отказывайтесь от металлических брекетов! Прозрачные элайнеры Invisalign предлагают практически невидимый, удобный способ достижения идеально ровных зубов. Изготовленные на заказ для точного соответствия, эти съемные элайнеры позволяют вам есть, пить и улыбаться с уверенностью — одновременно выравнивая ваши зубы без усилий. Получите улыбку, которую вы заслуживаете, современным способом!"
        : "Ditch the metal braces! Invisalign clear aligners offer a virtually invisible, comfortable way to achieve perfectly aligned teeth. Custom-made for a precise fit, these removable aligners let you eat, drink, and smile with confidence—while straightening your teeth effortlessly. Get the smile you deserve, the modern way!",
      categoryKey: "dental-treatments",
      path: "laser-gum-treatments",
      subCategoryKey: "periodontics",
    },
    {
      img: bonegrafting,
      title: isRu ? "Костная пересадка" : "Bone Grafting",
      description: isRu
        ? "Потеряли кость из-за отсутствующих зубов или заболеваний десен? Костная пересадка восстанавливает и укрепляет вашу челюсть, создавая прочную основу для зубных имплантатов и долгосрочного здоровья полости рта. Наши передовые техники обеспечивают безопасную, эффективную и комфортную процедуру — возвращая силу и функцию вашей улыбке!"
        : "Lost bone due to missing teeth or gum disease? Bone grafting rebuilds and strengthens your jaw, creating a solid foundation for dental implants and long-term oral health. Our advanced techniques ensure a safe, effective, and comfortable procedure—bringing back your smile’s strength and function!",
      categoryKey: "dental-treatments",
      path: "bone-grafting",
      subCategoryKey: "oral-surgery",
    },
    // eye
    // desis
    {
      img: diabetic,
      title: isRu
        ? "Лечение диабетической ретинопатии"
        : "Diabetic Retinopathy Treatment",
      description: isRu
        ? "Размытое зрение, катаракта или зависимость от очков и контактных линз ограничивают вашу повседневную жизнь? Откройте для себя потенциал имплантации интраокулярных линз (ИОЛ) в Турции, надежном направлении для передового ухода за глазами. AZPO Health может направить вас к изучению вариантов для более ясного зрения и светлого будущего."
        : "Are blurry vision, cataracts, or reliance on glasses and contacts limiting your daily life? Discover the potential of Intraocular Lens (IOL) implantation in Turkey, a trusted destination for advanced eye care. AZPO Health can guide you to explore options for clearer vision and a brighter future.",
      categoryKey: "eye-surgery",
      path: "diabetic-retinopathy-treatment",
      subCategoryKey: "eye-disease-treatments",
    },
    {
      img: glocama,
      title: isRu ? "Лечение глаукомы" : "Glaucoma treatment",
      description: isRu
        ? "Размытое зрение, давление в глазах или постепенная утрата зрения затрудняют повседневную жизнь? Глаукома — тихий вор зрения, но раннее вмешательство может спасти ваше зрение. Верните надежду с помощью передовых методов лечения глаукомы. AZPO Health соединяет вас с экспертными клиниками в Турции, предлагающими персонализированный уход для управления вашим состоянием и сохранения вашего драгоценного зрения, обеспечивая светлое будущее для вашего зрения."
        : "Is blurry vision, eye pressure, or gradual vision loss making daily life harder? Glaucoma is a silent thief of sight—but early intervention can save your vision. Regain hope with advanced Glaucoma Treatment options. AZPO Health connects you with expert clinics in Turkey offering personalized care to manage your condition and help preserve your precious sight, ensuring a brighter future for your vision.",
      categoryKey: "eye-surgery",
      path: "glaucoma-treatment",
      subCategoryKey: "eye-disease-treatments",
    },
    // surgical
    {
      img: intraocularImg,
      title: isRu ? "Интраокулярная линза" : "Intraocular Lens",
      description: isRu
        ? "Размытое зрение, катаракта или зависимость от очков и контактных линз ограничивают вашу повседневную жизнь? Откройте для себя потенциал имплантации интраокулярных линз (ИОЛ) в Турции, надежном направлении для передового ухода за глазами. AZPO Health может направить вас к изучению вариантов для более четкого зрения и светлого будущего."
        : "Are blurry vision, cataracts, or reliance on glasses and contacts limiting your daily life? Discover the potential of Intraocular Lens (IOL) implantation in Turkey, a trusted destination for advanced eye care. AZPO Health can guide you to explore options for clearer vision and a brighter future.",
      categoryKey: "eye-surgery",
      path: "intraocular-lens",
      subCategoryKey: "surgical-implant-procedures",
    },
    {
      img: corneal,
      title: isRu ? "Трансплантация роговицы" : "Corneal Transplantation",
      description: isRu
        ? "Чувствуете, что ваше зрение угасает — размытое, болезненное или просто не такое острое, как раньше? Поврежденная роговица может повлиять не только на ваше зрение, но и на вашу уверенность и повседневную жизнь. Но есть надежда. В ведущих медицинских центрах Турции квалифицированные хирурги используют передовые техники трансплантации роговицы для восстановления ясности и комфорта — даря вам дар четкого зрения снова."
        : "Does your vision feel like it’s fading—blurred, painful, or just not as sharp as it used to be? A damaged cornea can impact not just your sight, but your confidence and daily life. But there’s hope. In Turkey’s world-class medical centers, skilled surgeons use cutting-edge corneal transplantation techniques to restore clarity and comfort—giving you the gift of clear vision once again. Whether due to injury, disease, or degeneration, you don’t have to settle for compromised sight.",
      categoryKey: "eye-surgery",
      path: "corneal-transplantation",
      subCategoryKey: "surgical-implant-procedures",
    },
    // fertility Male-Infertility
    {
      img: maleFertility,
      title: isRu ? "Мужское бесплодие" : "Male infertility",
      description: isRu
        ? "Пытаетесь зачать ребенка, но сталкиваетесь с бесконечными разочарованиями? Мужское бесплодие — это скрытая борьба, которая затрагивает миллионы, но мало кто говорит об этом. Низкое количество сперматозоидов, плохая подвижность и гормональные дисбалансы могут подорвать уверенность и напрячь отношения. Но есть надежда — решения существуют. Не страдайте в молчании. Сделайте первый шаг сегодня."
        : "Trying for a baby but facing endless disappointment? Male infertility is a hidden struggle that affects millions, yet few talk about it. Low sperm count, poor motility, and hormonal imbalances can crush confidence and strain relationships. But there’s hope—solutions exist. Don’t suffer in silence. Take the first step today.",
      categoryKey: "fertility",
      path: "male-infertility",
      subCategoryKey: "fertility-assisted-reproduction",
    },
    // stem cell
    // male-sexual-health-issues
    {
      img: malesexual,
      title: isRu
        ? "Проблемы мужского сексуального здоровья"
        : "Male sexual health issues",
      description: isRu
        ? "Ваше сексуальное здоровье влияет на вашу уверенность, отношения и общее благополучие? Вы не одиноки — и реальные решения существуют. Будь то эректильная дисфункция, низкое либидо или проблемы с производительностью, передовые методы лечения в ведущих медицинских центрах Турции предлагают надежду и трансформацию. Начните свое путешествие с AZPO Health сейчас."
        : "Is your sexual health affecting your confidence, relationships, and overall well-being? You’re not alone—and real solutions exist. Whether it’s Erectile Dysfunction, low libido, or performance concerns, advanced treatments in Turkey’s world-class medical centers offer hope and transformation. Start your journey with AZPO health now.",
      categoryKey: "stem-cell",
      path: "male-sexual-health-issues",
      subCategoryKey: "male-sexual-therapy",
    },
    {
      img: peyroniesdisease,
      title: isRu ? "Болезнь Пейрони" : "Peyronies disease",
      description: isRu
        ? "Боретесь с дискомфортом, искривлением или проблемами с интимной жизнью? Болезнь Пейрони может быть разочаровывающей, но терапия стволовыми клетками в Турции предлагает революционное, нехирургическое решение. Это инновационное лечение уменьшает бляшки, восстанавливает естественную функцию и повышает уверенность с экспертным уходом. Сделайте первый шаг к облегчению и восстановлению сегодня."
        : "Struggling with discomfort, curvature, or intimacy challenges? Peyronie’s Disease can be frustrating—but Stem Cell Therapy in Turkey offers a groundbreaking, non-surgical solution. This innovative treatment reduces plaque, restores natural function, and improves confidence with expert care. Take the first step toward relief and recovery today.",
      categoryKey: "stem-cell",
      path: "peyronies-disease",
      subCategoryKey: "male-sexual-therapy",
    },
    {
      img: dyfunc,
      title: isRu ? "Эректильная дисфункция" : "Erectile Dysfunction ED",
      description: isRu
        ? "ЭД заставляет вас чувствовать себя расстроенным, оторванным или менее самим собой? Вы не одиноки — и вам не придется это принимать. Представьте, как вы восстанавливаете свои способности, уверенность и страсть с помощью передового решения, которое устраняет причину проблемы. В AZPO Health мы соединяем вас с передовыми методами лечения эректильной дисфункции в Турции, включая революционную терапию стволовыми клетками — предназначенную для естественного восстановления, регенерации и оживления вашей сексуальной функции. Больше никаких временных решений — только настоящие, длительные результаты."
        : "Does ED have you feeling frustrated, disconnected, or less like yourself? You’re not alone—and you don’t have to accept it. Imagine restoring your performance, confidence, and passion with a cutting-edge solution that targets the root cause. At AZPO Health, we connect you to advanced Erectile Dysfunction treatments in Turkey, including revolutionary Stem Cell Therapy—designed to naturally repair, regenerate, and revitalize your sexual function. No more temporary fixes—just real, lasting results.",
      categoryKey: "stem-cell",
      path: "erectile-dysfunction-ed",
      subCategoryKey: "male-sexual-therapy",
    },
    {
      img: stemmen,
      title: isRu
        ? "Терапия стволовыми клетками для мужчин"
        : "Stem cell for men",
      description: isRu
        ? "Чувствуете тяжесть возраста или специфических проблем со здоровьем, влияющих на ваше благополучие как мужчины? Представьте терапию, которая использует собственную целебную силу вашего тела для омоложения и решения этих проблем, предлагая новую жизненную энергию и надежду. Терапия стволовыми клетками, предоставляемая AZPO Health в Турции, может быть инновационным решением, которое вы искали для более здорового будущего."
        : "Feeling the weight of age or specific health challenges affecting your well-being as a man? Imagine a therapy that harnesses your body's own healing power to rejuvenate and address these concerns, offering a renewed sense of vitality and hope. stem cell therapy, facilitated by AZPO Health in Turkey, could be the innovative solution you've been seeking for a healthier future.",
      categoryKey: "stem-cell",
      path: "stem-cell-for-men",
      subCategoryKey: "therapy-regenerative-medicine",
    },
    {
      img: orthopedic,
      title: isRu
        ? "Ортопедическая терапия стволовыми клетками"
        : "Orthopedic stem cell therapy",
      description: isRu
        ? "Что, если бы вы могли исцелить суставы естественно — без хирургии? Ортопедическая терапия стволовыми клетками использует регенеративную силу вашего тела для восстановления хряща, уменьшения боли и восстановления подвижности. Будь то артрит, спортивные травмы или хроническая боль, это передовое лечение возвращает вас к движению — сильнее, быстрее, без боли!"
        : "What if you could heal your joints naturally—without surgery? Orthopedic Stem Cell Therapy uses your body’s own regenerative power to repair cartilage, reduce pain, and restore mobility. Whether it’s arthritis, sports injuries, or chronic pain, this cutting-edge treatment gets you moving again—stronger, faster, pain-free!",
      categoryKey: "stem-cell",
      path: "orthopedic-stem-cell-therapy",
      subCategoryKey: "therapy-regenerative-medicine",
    },
    {
      img: neurological,
      title: isRu ? "Неврологическая терапия" : "Neurological therapy",
      description: isRu
        ? "Вы или ваш близкий сталкиваетесь с трудностями неврологического состояния, мечтая о достижениях, которые дают надежду? В Турции вы можете найти доступ к клиникам, специализирующимся на терапии стволовыми клетками для неврологических расстройств, возможно, предлагая путь к улучшению благополучия и светлому будущему. Свяжитесь с AZPO Health, чтобы изучить варианты подключения к этим специализированным медицинским центрам."
        : "Are you or a loved one facing the challenges of a neurological condition, longing for advancements that offer a ray of hope? In Turkey, you can find access to clinics specializing in stem cell therapies for neurological disorders, potentially offering a path toward improved well-being for a brighter future. Contact AZPO Health to explore your options for connecting with these specialized medical centers.",
      categoryKey: "stem-cell",
      path: "neurological-therapy",
      subCategoryKey: "therapy-regenerative-medicine",
    },
    // tipplasty
    {
      img: tipplasty,
      title: isRu ? "Типпластика" : "Tip plasty",
      description: isRu
        ? "Если кончик вашего носа кажется неуместным, слишком круглым, выпуклым или просто несбалансированным, типпластика может уточнить его без полной ринопластики. В Турции ведущие специалисты предлагают эту точную коррекцию, принося гармонию вашим чертам с тонким, но мощным результатом. Через надежную сеть AZPO Health вы получите экспертный уход за естественной, повышающей уверенность трансформацией."
        : "If your nasal tip feels off, too round, too bulbous, or just unbalanced, Tip Plasty can refine it without a full rhinoplasty. In Turkey, top specialists offer this precise enhancement, bringing harmony to your features with subtle yet powerful results. Through AZPO Health’s trusted network, you’ll get expert care for a natural, confidence-boosting transformation.",
      categoryKey: "aesthetic",
      path: "tip-plasty",
      subCategoryKey: "facial-aesthetic-surgery",
    },
    // Almond Eye
    {
      img: almondeye,
      title: isRu ? "Миндалевидные глаза" : "Almond Eye",
      description: isRu
        ? "Желаете привлекательных, элегантных глаз, которые усиливают вашу естественную красоту и излучают очарование? В Турции вы можете получить экспертный уход и передовые техники для достижения желаемой миндалевидной формы глаз, повышая вашу уверенность и трансформируя ваш взгляд с естественными результатами."
        : "Are you longing for captivating, elegant eyes that enhance your natural beauty and radiate allure? In Turkey, you can access expert care and advanced techniques to achieve the sought-after almond eye shape, boosting your confidence and transforming your look with natural-looking results.",
      categoryKey: "aesthetic",
      path: "almond-eye",
      subCategoryKey: "facial-aesthetic-surgery",
    },
    // face lift
    {
      img: facelift,
      title: isRu ? "Подтяжка лица" : "Face lift",
      description: isRu
        ? "Признаки старения на лице, такие как обвисшая кожа, морщины и усталый вид, подрывают вашу уверенность? Представьте себе обновленный и более молодой вид. AZPO Health соединяет вас с опытными медицинскими специалистами в Турции, которые специализируются на процедурах подтяжки лица, помогая вам достичь ваших эстетических целей с персонализированным уходом и многообещающими результатами. Свяжитесь с нами сегодня для бесплатной консультации."
        : "Are the signs of aging on your face like sagging skin, wrinkles, and a tired appearance affecting your confidence? Imagine a refreshed and more youthful look. AZPO Health connects you with experienced medical professionals in Turkey who specialize in Facelift procedures, helping you achieve your aesthetic goals with personalized care and promising results. Contact us today for a free consultation.",
      categoryKey: "aesthetic",
      path: "face-lift",
      subCategoryKey: "facial-aesthetic-surgery",
    },
    // bichecktomy
    {
      img: bichecktomy,
      title: isRu ? "Биэктомия" : "Bichecktomy",
      description: isRu
        ? "Хотите уточнить внешний вид вашего лица и достичь более стройных, скульптурных щек? В Турции вы можете найти доступ к опытным профессионалам, которые выполняют биэктомию, процедуру, направленную на уменьшение объема щек. Свяжитесь с квалифицированными клиниками через AZPO Health и узнайте, как это лечение может помочь вам достичь более четкого и сбалансированного лица."
        : "Are you looking to refine your facial appearance and achieve slimmer, more sculpted cheeks? In Turkey, you can find access to experienced professionals who perform bichectomy, a procedure aimed at reducing fullness in the cheeks. Connect with qualified clinics through AZPO Health and explore how this treatment can help you attain a more defined and balanced facial aesthetic.",
      categoryKey: "aesthetic",
      path: "bichecktomy",
      subCategoryKey: "facial-aesthetic-surgery",
    },
    // mommy-makeover
    {
      img: momymakeover,
      title: isRu ? "Мамина реконструкция" : "Mommy makeover",
      description: isRu
        ? "Чувствуете влияние материнства на ваше тело, замечая изменения, которые снижают вашу уверенность? Мамина реконструкция в Турции с AZPO Health предлагает комплексное решение для омоложения вашего внешнего вида, помогая вам снова почувствовать себя собой. Исследуйте возможности и сделайте первый шаг к обновленной уверенности."
        : "Are you feeling the impact of motherhood on your body, noticing changes that leave you less confident? A mommy makeover in Turkey with AZPO health offers a comprehensive solution to rejuvenate your appearance, helping you feel like yourself again. Explore the possibilities and take the first step towards renewed confidence.",
      categoryKey: "aesthetic",
      path: "mommy-makeover",
      subCategoryKey: "body-contouring",
    },
    // breast-reduction
    {
      img: breastreduc,
      title: isRu ? "Уменьшение груди" : "Breast reduction",
      description: isRu
        ? "Чувствуете бремя физического и эмоционального веса слишком больших грудей? Дискомфорт, боль или ограничения в повседневной жизни становятся постоянной борьбой? В Турции вы можете найти опытных хирургов и современные учреждения, которые специализируются на операции по уменьшению груди, предлагая путь к обновленному комфорту и улучшенному благополучию. Свяжитесь с AZPO Health сегодня, чтобы изучить ваши варианты и сделать первый шаг к более сбалансированной и уверенной версии себя."
        : "Feeling burdened by the physical and emotional weight of overly large breasts? Is discomfort, pain, or limitations in your daily life becoming a constant struggle? In Turkey, you can find experienced surgeons and modern facilities that specialize in breast reduction surgery, offering a potential pathway to renewed comfort and improved well-being. Contact AZPO Health today to explore your options and take the first step towards a more balanced and confident you.",
      categoryKey: "aesthetic",
      path: "breast-reduction",
      subCategoryKey: "breast-surgery",
    },
    // Male breast reduction
    {
      img: malebreastreduc,
      title: isRu ? "Уменьшение груди у мужчин" : "Male breast reduction",
      description: isRu
        ? "Устали прятаться под мешковатой одеждой? Чувствуете неловкость каждый раз, когда снимаете рубашку? Вы не одиноки — но вам не придется так жить. В AZPO Health мы соединяем вас с ведущими хирургами в Турции, специализирующимися на уменьшении груди у мужчин, чтобы вы могли получить скульптурную, мужественную грудь и обрести уверенность, чувствуя себя комфортно в своем теле."
        : "Tired of hiding under baggy clothes? Feeling self-conscious every time you take off your shirt? You’re not alone—but you don’t have to live like this. At AZPO Health, we connect you with top-tier surgeons in Turkey specializing in male breast reduction—giving you a sculpted, masculine chest so you can walk with confidence and finally feel at home in your own body.",
      categoryKey: "aesthetic",
      path: "male-breast-reduction",
      subCategoryKey: "breast-surgery",
    },
    // Vaginal tightening
    {
      img: veginal,
      title: isRu ? "Вагинальное омоложение" : "Vaginal tightening",
      description: isRu
        ? "Чувствуете, что что-то изменилось? Потеря упругости, дискомфорт или снижение уверенности влияют на вашу интимную жизнь? Вы не одиноки — и вам не придется мириться с этим. В AZPO Health мы соединяем вас с ведущими специалистами в Турции, предлагающими передовые процедуры вагинального омоложения (вагинопластики), разработанные для восстановления комфорта, уверенности и удовольствия."
        : "Do you feel like something’s changed? A loss of tightness, discomfort, or a dip in confidence affecting your intimate life? You’re not alone—and you don’t have to settle. At AZPO Health, we connect you with leading specialists in Turkey offering advanced vaginal tightening (vaginoplasty) treatments designed to restore comfort, confidence, and pleasure.",
      categoryKey: "aesthetic",
      path: "vaginal-tightening",
      subCategoryKey: "genital-aesthetic",
    },
    // Implant treatment
    {
      img: implant,
      title: isRu ? "Имплантация зубов" : "Implant treatment",
      description: isRu
        ? "Отсутствие зубов не должно определять вашу жизнь — будь то еда, разговор или просто улыбка без беспокойства. А что, если вы могли бы вернуть внешний вид и ощущение естественных зубов с помощью бесшовного и долговечного решения? В AZPO Health мы соединяем вас с элитными стоматологами в Турции, предлагающими передовые имплантационные процедуры, которые восстановят вашу улыбку по цене, значительно ниже, чем в других местах, без компромиссов в качестве."
        : "Missing teeth shouldn’t dictate your life—whether it’s eating, speaking, or simply smiling without worry. But what if you could get back the look and feel of natural teeth with a solution that’s seamless and lasting? At AZPO Health, we connect you with elite dental experts in Turkey, offering cutting-edge implant treatments that restore your smile at a fraction of the price you’d pay elsewhere—without compromising on quality.",
      categoryKey: "dental-treatments",
      path: "implant-treatment",
      subCategoryKey: "implantology",
    },
    // fillings
    {
      img: fillings,
      title: isRu ? "Зубные пломбы" : "Fillings",
      description: isRu
        ? "Испытываете чувствительность зубов или дискомфорт от кариеса? Беспокоитесь о внешнем виде поврежденных или кариозных зубов, влияющих на вашу улыбку? В Турции вы можете найти высококачественные решения с передовыми процедурами пломбирования зубов, которые восстановят здоровье полости рта и вернут вам уверенную улыбку. AZPO Health соединяет вас с опытными стоматологами, готовыми предоставить необходимый уход."
        : "Are you experiencing tooth sensitivity or discomfort from cavities? Concerned about the appearance of damaged or decayed teeth affecting your smile? In Turkey, you can find high-quality solutions with advanced dental filling procedures that can restore your oral health and bring back your confident smile. AZPO Health connects you with experienced dental professionals ready to provide the care you need.",
      categoryKey: "dental-treatments",
      path: "fillings",
      subCategoryKey: "general-dentistry",
    },
    // routine check ups
    {
      img: rotinecheckupscleaning,
      title: isRu
        ? "Регулярные осмотры и чистка"
        : "Routine Check-ups & Cleanings",
      description: isRu
        ? "Беспокоитесь о поддержании здоровья полости рта без лишних затрат? Желаете тщательных стоматологических осмотров, которые ставят ваше благополучие на первое место? Турция предлагает убедительное решение, сочетая исключительный стоматологический уход с экономичными вариантами и незабываемым отдыхом во время лечения зубов. Ваша улыбка получит экспертное внимание, которого она заслуживает, с AZPO Health."
        : "Are you concerned about maintaining your oral health without straining your budget? Longing for thorough dental evaluations that put your well-being first? Turkey offers a compelling solution, blending exceptional dental care with cost-effective options and above all a memorable holiday too while doing your dental work, your smile receives the expert attention it deserves with AZPO Health.",
      categoryKey: "dental-treatments",
      path: "routine-check-ups",
      subCategoryKey: "general-dentistry",
    },
    // dental -x- rays
    {
      img: xrays,
      title: isRu ? "Стоматологические рентгены" : "Dental X-rays",
      description: isRu
        ? "Беспокоитесь о стоматологической проблеме, но не уверены в следующем шаге? В Турции доступ к важным диагностическим инструментам, таким как стоматологические рентгены, прост и предоставляет четкую картину вашего здоровья полости рта, открывая путь к эффективным и успокаивающим решениям. Свяжитесь с AZPO Health сегодня, чтобы найти опытных стоматологов."
        : "Concerned about a dental issue but unsure of the next step? In Turkey, accessing essential diagnostic tools like dental X-rays is straightforward and provides a clear picture of your oral health, paving the way for effective and reassuring solutions. Contact AZPO Health today to connect with experienced dental professionals.",
      categoryKey: "dental-treatments",
      path: "dental-x-rays",
      subCategoryKey: "general-dentistry",
    },
    // Tooth Extractions
    {
      img: toothextraction,
      title: isRu ? "Удаление зубов" : "Tooth Extractions",
      description: isRu
        ? "Испытываете постоянную зубную боль или дискомфорт, возможно, из-за сильно поврежденного или кариозного зуба? Мысль об удалении может пугать, но в Турции вы можете найти опытных стоматологов, которые предлагают мягкие и эффективные процедуры удаления зубов, открывая путь к более здоровой и комфортной улыбке. Свяжитесь с AZPO Health сегодня, чтобы найти надежные клиники для вашего персонализированного ухода."
        : "Are you experiencing persistent tooth pain or discomfort, perhaps from a severely decayed or damaged tooth? The thought of extraction might seem daunting, but in Turkey, you can find experienced dental professionals who offer gentle and effective tooth extractions, paving the way for a healthier and more comfortable smile. Contact AZPO Health today to connect with reputable clinics for your personalized care.",
      categoryKey: "dental-treatments",
      path: "tooth-extractions",
      subCategoryKey: "general-dentistry",
    },
    // wisdom teeth removal
    {
      img: wisdom,
      title: isRu ? "Удаление зубов мудрости" : "Wisdom Teeth Removal",
      description: isRu
        ? "Испытываете дискомфорт, боль или отек в задней части рта? Возможно, ваш стоматолог упомянул необходимость удаления зубов мудрости. Многие сталкиваются с этой распространенной стоматологической проблемой, и хорошая новость в том, что эффективные и качественные решения доступны. Рассмотрение вариантов для вашего здоровья полости рта может быть ошеломляющим, но изучение преимуществ удаления зубов мудрости в Турции через AZPO Health может обеспечить вам комфортное и поддерживаемое путешествие к более здоровой улыбке."
        : "Are you experiencing discomfort, pain, or swelling in the back of your mouth? Perhaps your dentist has mentioned the need for wisdom teeth removal. Many people face this common dental concern, and the good news is that effective and high-quality solutions are available. Considering options for your oral health can feel overwhelming, but exploring the benefits of having your wisdom teeth removal in Turkey through AZPO Health can provide you with a comfortable and supported journey towards a healthier smile.",
      categoryKey: "dental-treatments",
      path: "wisdom-teeth-removal",
      subCategoryKey: "oral-surgery",
    },
    // jaw surgery
    {
      img: jawsurgery,
      title: isRu ? "Челюстная хирургия" : "Jaw Surgery",
      description: isRu
        ? "Испытываете дискомфорт, трудности с жеванием или беспокоитесь о выравнивании лица? Челюстная хирургия в Турции предлагает трансформирующее решение, и AZPO Health может соединить вас с опытными профессионалами, чтобы направить вас к улучшенной функции и обновленной уверенности."
        : "Are you experiencing discomfort, difficulty chewing, or concerns about your facial alignment? Jaw surgery in Turkey offers a transformative solution, and AZPO Health can connect you with experienced professionals to guide you toward improved function and renewed confidence.",
      categoryKey: "dental-treatments",
      path: "jaw-surgery",
      subCategoryKey: "oral-surgery",
    },
    // Cataract Treatment
    {
      img: cataract,
      title: isRu ? "Лечение катаракты" : "Cataract Treatment",
      description: isRu
        ? "Мутное зрение и трудности с четким видением усложняют повседневную жизнь? Не отчаивайтесь! В Турции вы можете найти экспертный уход и передовые варианты лечения катаракты, предлагающие обновленный взгляд с более четким зрением и светлым будущим. Свяжитесь с AZPO Health, чтобы изучить ваш персонализированный путь к ясности зрения."
        : "Are cloudy vision and difficulty seeing clearly making everyday life a challenge? Take heart! In Turkey, you can find expert care and advanced options for cataract treatment, offering a renewed outlook with clearer vision and a brighter future. Contact AZPO Health to explore your personalized path to visual clarity.",
      categoryKey: "ophthalmology-eye-surgery",
      path: "cataract-treatment",
      subCategoryKey: "eye-disease-treatments",
    },
    // retinal-disease-treatment
    {
      img: retinaldisease,
      title: isRu
        ? "Лечение заболеваний сетчатки"
        : "Retinal Disease Treatment",
      description: isRu
        ? "Изменения в зрении из-за состояния сетчатки могут быть тревожными, влияя на вашу повседневную жизнь и общее благополучие. Ищете специализированный уход и эффективные решения для вашего заболевания сетчатки? В Турции вы можете найти высококвалифицированных медицинских специалистов и передовые варианты лечения, и AZPO Health здесь, чтобы сопровождать вас на каждом этапе вашего пути."
        : "Experiencing changes in your vision due to a retinal condition can be unsettling, affecting your daily life and overall well-being. Are you looking for specialized care and effective solutions for your retinal disease? In Turkey, you can find highly skilled medical professionals and advanced treatment options, and AZPO Health is here to guide you through every step of your journey.",
      categoryKey: "ophthalmology-eye-surgery",
      path: "retinal-disease-treatment",
      subCategoryKey: "eye-disease-treatments",
    },
    // LASIK surgery
    {
      img: lasiksurgery,
      title: isRu ? "Лазерная хирургия LASIK" : "LASIK surgery",
      description: isRu
        ? "Устали от ежедневной борьбы с очками или дискомфорта от контактных линз? Представьте, что вы просыпаетесь каждое утро с четким, ясным зрением. В Турции передовая лазерная хирургия LASIK предлагает безопасный и эффективный путь к свободе зрения, и AZPO Health здесь, чтобы соединить вас с ведущими клиниками для изучения этой изменяющей жизнь возможности."
        : "Are you tired of the daily struggle with glasses or the discomfort of contact lenses? Imagine waking up every morning with clear, crisp vision. In Turkey, advanced LASIK eye surgery offers a safe and effective path to visual freedom, and AZPO Health is here to connect you with leading clinics to explore this life-changing possibility.",
      categoryKey: "ophthalmology-eye-surgery",
      path: "lasik-surgery",
      subCategoryKey: "surgical-implant-procedures",
    },
    // Smart lens
    {
      img: smartlens,
      title: isRu ? "Умные линзы" : "Smart Lens",
      description: isRu
        ? "Устали от борьбы с размытым зрением, постоянного поиска очков для чтения или чувства ограниченности из-за вашего зрения? Представьте жизнь с более четким зрением на всех расстояниях, возможно, освободив вас от ежедневных хлопот с очками и контактными линзами. Технология умных линз в Турции предлагает многообещающее решение, и AZPO Health может соединить вас с опытными профессионалами для изучения этой изменяющей жизнь возможности."
        : "Are you tired of struggling with blurry vision, constantly reaching for your reading glasses, or feeling limited by your eyesight? Imagine a life with sharper vision at all distances, potentially freeing you from the daily hassle of glasses and contact lenses. Smart lens technology in Turkey offers a promising solution, and AZPO Health can connect you with experienced professionals to explore this life-changing opportunity.",
      categoryKey: "ophthalmology-eye-surgery",
      path: "smart-lens",
      subCategoryKey: "surgical-implant-procedures",
    },
    // IVF
    {
      img: ivf,
      title: isRu ? "ЭКО (экстракорпоральное оплодотворение)" : "IVF",
      description: isRu
        ? "Мечтаете о создании семьи, но сталкиваетесь с трудностями в зачатии? ЭКО в Турции предлагает надежду с передовыми репродуктивными технологиями и заботливым уходом. AZPO Health соединяет вас с ведущими клиниками фертильности, обеспечивая персонализированные планы лечения и поддержку на каждом этапе вашего пути к родительству."
        : "Dreaming of starting a family but facing challenges with conception? IVF in Turkey offers hope with advanced reproductive technologies and compassionate care. AZPO Health connects you with leading fertility clinics, providing personalized treatment plans and support every step of the way toward parenthood.",
      categoryKey: "fertility",
      path: "ivf",
      subCategoryKey: "fertility-assisted-reproduction",
    },
    // FET
    {
      img: fet,
      title: isRu
        ? "Перенос замороженных эмбрионов (FET)"
        : "FET (Frozen Embryo Transfer)",
      description: isRu
        ? "Мечтаете о ребенке, но сталкиваетесь с трудностями при естественном зачатии или предыдущих репродуктивных процедурах? Может ли ответ на ваши надежды лежать в мягком, но высокоэффективном подходе в стране, известной своей медицинской экспертизой? Изучите возможность переноса замороженных эмбрионов в Турции, и пусть AZPO Health направит вас к светлому будущему."
        : "Are you dreaming of holding your baby but facing challenges with natural conception or previous fertility treatments? Could the answer to your hopes lie in a gentle, yet highly effective approach in a welcoming land renowned for its medical expertise? Explore the possibility of frozen embryo transfer in Turkey, and let AZPO Health guide you towards a brighter future.",
      categoryKey: "fertility",
      path: "fet-frozen-embryo-transfer",
      subCategoryKey: "fertility-assisted-reproduction",
    },
  ];
  return treatments;
};
